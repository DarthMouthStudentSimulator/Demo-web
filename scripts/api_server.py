...
if __name__ == "__main__":
    ...
    # Get host and port from environment variables for Codespaces
    host = os.getenv("HOST", "0.0.0.0")  # Use 0.0.0.0 for Codespaces
    port = int(os.getenv("PORT", "8089"))
    
    print(f"🚀 Starting API server on {host}:{port}")
    print(f"📊 API Documentation: http://{host}:{port}/docs")
    print(f"🔗 Health check: http://{host}:{port}/api/users")
